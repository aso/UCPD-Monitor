...
STM32CubeMonitor-UCPD のトレース機能を代替します。STM32CubeMonitor-UCPD が出力する `.cpd` バイナリファイルをデコードして表示もできます。
...
本プロジェクトはこの制限を解消するために開発されました。STM32CubeMonitor-UCPD が出力する `.cpd` バイナリストリームをそのまま受け取り、**USB PD Rev 3.2 準拠の独自デコーダ**でログ表示します。

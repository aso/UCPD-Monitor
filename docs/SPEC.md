...
| メソッド | パス | 説明 |
|---|---|---|
| GET | `/api/status` | サーバ稼働確認。`{status, appId, clients}` を返す。`appId` は `'ucpd-monitor-server'` 固定で自己同一性の確認に使用 |
| GET | `/api/serial/ports` | 利用可能なシリアルポート一覧 (SerialPort.list()) |
| POST | `/api/import-cpd` | バイナリ `.cpd` データ受信・解析・HISTORY ブロードキャスト |
| POST | `/api/shutdown` | **開発モードのみ** — プロセスを終了して同一ポートへの新インスタンス起動を可能にする。Electron ビルドでは 403 を返す |

> **注:** シリアルポートの接続・切断・keepalive は WebSocket メッセージで処理する（REST エンドポイントなし）。
...
```powershell
npm run dev:web
# concurrently でサーバ (port 57321, UCPD_PORT) + Vite (port 5173) を起動
# カスタムポート: UCPD_PORT=12345 npm run dev:web
```
...
Expressサーバ（ポート `57321`、`UCPD_PORT` 環境変数で変更可）と Vite 開発サーバ（ポート 5173）を同時起動します。ブラウザで `http://localhost:5173` を開いてください。
...
サーバはローカル開発用ツールであるが、ブラウザから利用する場合の攻撃経路を最小化するため、以下の多層防御を実装している。
...
`server.listen` の第 2 引数を `'127.0.0.1'` に固定し、外部ネットワークインターフェースへの公開を防ぐ。

### 14.2 HTTP Host ヘッダー検証（DNS Rebinding 対策）

全 HTTP リクエストに対し、`Host` ヘッダーのホスト部分が `localhost` または `127.0.0.1` であることを検証する Express ミドルウェアを挿入している。

```
攻撃シナリオ: 悪意ある Web ページが attacker.com の DNS を 127.0.0.1 に書き換えた後、
              fetch('http://attacker.com:57321/api/shutdown') を呼ぶ
対策: Host ヘッダーが 'attacker.com' → 403 Forbidden
```

### 14.3 WebSocket Origin 検証

`WebSocketServer` の `verifyClient` コールバックで、接続元 Origin が `localhost` または `127.0.0.1` のいずれかであることを確認する。Electron renderer は Origin ヘッダーを送信しないため、`!origin` を許可条件とする。

### 14.4 `/api/shutdown` の Electron ビルドでの無効化

`process.versions.electron` が truthy な場合（Electron 本番ビルド）は `POST /api/shutdown` に対して 403 を返す。これにより、エンドユーザー環境でのリモートシャットダウンを防ぐ。開発モードでのみ有効（新起動インスタンスによる古いインスタンスの安全な置換に使用）。

### 14.5 EADDRINUSE 時の安全な自己置換（開発モードのみ）

ポートが既に使用中の場合、以下の順序でのみ自己置換を許可する:

1. OS netstat/lsof でプロセス名確認 → `node` / `node.exe` のみ続行
2. `GET /api/status` で `appId: 'ucpd-monitor-server'` を確認
3. `POST /api/shutdown` で既存プロセスを終了
4. 800ms 待機後、同一ポートで再バインド

Electron ビルドでは上記プロセスを実施せず即 reject し、ポート競合をエラーとして扱う。

### 14.6 ポート番号の選択

デフォルトポートを `57321` (Dynamic/Private port range: 49152–65535) に設定し、一般的なサービスとの衝突を回避する。環境変数 `UCPD_PORT` で変更可能。
